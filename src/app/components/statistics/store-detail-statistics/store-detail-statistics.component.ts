...
@Component({
  selector: 'app-store-detail-statistics',
  templateUrl: './store-detail-statistics.component.html',
  styleUrls: ['./store-detail-statistics.component.scss'],
})
  ...
  updateDailyFootprintChart(locationId: string) {
    this.userRouteService.getAllUserRoutes().subscribe((userRoutes: any) => {
      ...
      this.routeService.getAllRoutes().subscribe((routes: any) => {
        ...
        // Extract route times and sort them in ascending order
        const routeTimes = filteredUserRoutes
          .map(
            (userRoute: any) =>
        ...
        // Extract labels and data from dailyCounts
